...
"""
FICH_MASTER = MASTER_RESULTS+"/ordenes_master.txt"
...
def generarAjuste(fich_ordenes, fich_conf):
    ...
    for i in range(16):
        ...
        #plt.plot(XX,YY)
        #for i in posiciones: plt.axvline(np.float(i),ls=':',c='red')
        for y0 in posiciones:
            y0 = np.int(np.float(y0))
            x=XX[y0-8:y0+8]
            y=YY[y0-8:y0+8] 
            #print y0         
            #plt.plot(x,y)
            ...
            try:
                ...
                print "Rutina 2 WARNING: el flat "+fich_ordenes+" no se ajustó en X="+str(posX)+" Y="+str(y0)
            
        #for i in newPos: plt.axvline(np.float(i),ls=':',c='blue')
        #plt.show()
        #sys.exit()
...
def rutina02Run(listaFlat):
    ...
    for line in infile:
        ...
        if len(line)>0:
            ...
            date = line[-21:-15]
            nomFichero = line[-14:-5]+"_"+date+"_dat.txt"
            escribirMatriz(matPos,TMP_RESULTS+"/Rut02_dat/"+nomFichero)
...
def checkRutina02(listaAjustes, listaFlat):
    ...
    if desvMedia10 < 0.1 and desvMedia10 > -0.1:
        print colored("... Desviación media del orden 10: %.2f pix ... OK"%(desvMedia10), 'green')
    else:
        print colored("... Desviación media del orden 10: %.2f pix ... NO OK! - CHECK"%(desvMedia10), 'red')
    
    if desvMedia40 < 0.1 and desvMedia40 > -0.1:
        print colored("... Desviación media del orden 40: %.2f pix ... OK"%(desvMedia40), 'green')
    else:
        print colored("... Desviación media del orden 40: %.2f pix ... NO OK! - CHECK"%(desvMedia40), 'red')
    
    if desvMedia70 < 0.1 and desvMedia70 > -0.1:
        print colored("... Desviación media del orden 70: %.2f pix ... OK"%(desvMedia70), 'green')
    else:
        print colored("... Desviación media del orden 70: %.2f pix ... NO OK! - CHECK"%(desvMedia70), 'red')
...
def plotHistory():
    ...
    table = ascii.read(MASTER_RESULTS+'/ordenes_master.txt', format='csv', names=colnames, comment='@')	
